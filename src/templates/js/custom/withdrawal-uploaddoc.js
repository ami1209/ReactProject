...
$(document).ready(function () {
    $('#upload-single-doc').change(function() {
        ...
        if($("#single-doc-file").val() || $("#single-doc-file-back").val()) 
        {
            $("#single-doc-file").val("");
            $("#single-doc-file-back").val("");
            
            $('#single-document-preview-front span.document_name').html('');
            $('#single-document-preview-back span.document_name').html('');
            $('#single-document-preview-front').css("display", "none");
            $('#single-document-preview-back').css("display", "none");
            $('#single-doc-upload-btn').css('display','none');
            
        }

        $('#aadhar-div').css("display", "none");
        $('#aadhar-verify').css("display", "none");

        if($('#upload-single-doc').val() == "default") {
            $('#single-doc-browse').css("display", "none");
            $('#front-side-input').css("display", "none");
			$('#front-side-input').parent().css("display", "none");
			$('#back-side-input').css("display", "none");
			$('#single-doc-browse-back').css("display", "none");
            return false;
        }
        else if($('#upload-single-doc').val() == "AADHAR_API") {

            $('#single-doc-browse').css("display", "none");
            $('#aadhar-div').css("display", "block");
            $('#aadhar-verify').css("display", "block");

            $('#front-side-input').parent().css("display", "none");
            $('#back-side-input').parent().css("display", "none");
            ...
            return false;
               ...
               $('#front-side-input').parent().css("display", "block");
               $('#back-side-input').parent().css("display", "block");
               
               $('#front-side-input').css("display", "block");
               $('#back-side-input').css("display", "block");

               $('#single-doc-browse').css("display", "block");
               $('#single-doc-browse-back').css("display", "block");
        }
    });
    ...
    $('#single-doc-file').bind('change', function(event) {
        ...
        $('#single-document-preview-front').css("display", "block");
        $('#front-side-input').css("display", "none");

        $('#single-document-preview-front span.document_name').html('<strong>'+fileName+'</strong>');

        if(fileName !='')
        $('#single-doc-browse').css("display", "none");
        // $('#single-doc-select').css("display", "none");
        //$('#single-doc-upload-btn').css("display", "block");
        // if($('#single-doc-browse-back').css('display') == 'none')
            $('#single-doc-upload-btn').css("display", "block");
    });

    $('#single-doc-file-back').bind('change', function(event) {
        if($(this).val() == "")
            return false;

        if(validateFile("#single-doc-delete-back", "upload-single-doc", this, event.target) == false)
            return false;
        ...
        $('#single-document-preview-back').css("display", "block");
        $('#back-side-input').css("display", "none");

        $('#single-document-preview-back span.document_name').html('<strong>'+fileName+'</strong>');
        
        if(fileName !='')
        $('#single-doc-browse-back').css("display", "none");
        // $('#single-doc-select').css("display", "none");
        if($('#single-doc-browse').css('display') == 'none')
        $('#single-doc-upload-btn').css("display", "block");
    });
    ...
    $('#single-doc-edit-back').click(function() {
        $( "#single-doc-file-back" ).trigger( "click" );
    });
    ...
    $('#single-doc-delete-back').click(function() {
        $('#single-document-preview-back span.document_name').html('');
        $('#single-document-preview-back').css("display", "none");
        //$('#single-document-preview-back').parent().css("display", "none");
    ...
    $('#single-doc-upload-btn').click(function() {
        ...
        formData.append('file1', $('#single-doc-file')[0].files[0]);
        formData.append('file2', $('#single-doc-file-back')[0].files[0]);
    ...
    $("#aadharNo").on('keyup blur', function (e) {
        ...
        if ($(this).val().trim().length == 12) {
            ...
            var params = 'adhaarDataStr=' +$(this).val().trim();

            var key = e.which ? e.which : e.keyCode;
            if( key == 13 )
                params += "&triggerVerify=true";
            else
                params += "&triggerVerify=false";

            startAjax("/component/weaver/?task=withdrawal.checkAadharNoValid", params, checkAadharNoValid, "nottoshow");
        }
        else if(!$("#aadhar-verify").hasClass('disabled'))
        {
            $("#aadhar-verify").addClass("disabled");
        }
...
$(document).keypress(function(e) {
    if(e.which == 13) {
        if( $("#upload-single-doc").val() == "AADHAR_API" )
        {
            e.preventDefault();
            e.stopPropagation();
            e.stopImmediatePropagation();
...
function openGateway() {
    ...
    if($("#aadhar-verify").hasClass('disabled'))
        return false;

    removeToolTipErrorManual('all');
    var aadharNo = $("#aadharNo").val().trim();
    var aadharValidate = /^[0-9]{12}$/;
    if( aadharValidate.test(aadharNo) )
    {
        var params = 'aadharNo='+$("#aadharNo").val();
        startAjax("/component/weaver/?task=withdrawal.aadharGatewayInit",params,parseAadharInitResponse,"null");
    }
    else
    {
        showToolTipErrorManual("aadharNo", "Invalid Aadhar No. ", "bottom", $("#aadharNo"), undefined);
        return false;
    }
...
function parseAadharInitResponse(result) {
    ...
    {
        showToolTipErrorManual("aadharNo", res.message, "bottom", $("#aadharNo"), undefined);
        return false;
    }

    var myAadhaarGateway = new AadhaarAPIGateway(res.id, gatewayOptions);
    openAadhaarGateway(myAadhaarGateway);
}

function checkAadharNoValid(result) {
    ...
    if( res.errorCode != 0 )
    {
        ...
        showToolTipErrorManual("aadharNo", res.respMsg, "bottom", $("#aadharNo"), undefined);
        $("#aadhar-verify").removeClass('active');
        $("#aadhar-verify").addClass('disabled');
    ...
    {
        $("#aadhar-verify").removeClass('disabled');
        $("#aadhar-verify").addClass('active');

        if( res.triggerVerify == "true" )
            $("#aadhar-verify").trigger('click');
