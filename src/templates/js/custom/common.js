...
var websiteWindow,cashierpopup, rummypopup,referFriedPopup,takephoto,lastActiveModal = "";$ = jQuery.noConflict();
var eventName="";
var allUserEventLiableEvents = ['OPEN_CASHIER_WINDOW', 'OPEN_MOBILE_RUMMY_WINDOW', 'OPEN_RUMMY_WINDOW'];

function clearSystemMessage() {
    $("#system-message-container").empty()
}
function error_message(e, o) {
    $("#system-message-container").html("<div id='system-message'><div class='alert alert-danger'><a class='close' data-dismiss='alert'>X</a><h4 class='alert-heading'>Error</h4><div><p>" + e + "</p></div></div></div>"),
        $("#" + o).focus(),
        window.scrollTo(0, 0)
}
function warning_message(e) {
    $("#system-message-container").html("<div id='system-message'><div class='alert alert-warning'><a class='close' data-dismiss='alert'>X</a><h4 class='alert-heading'>Warning</h4><div><p>" + e + "</p></div></div></div>"),
        window.scrollTo(0, 0)
}
function info_message(e) {
    $("#system-message-container").html("<div id='system-message'><div class='alert alert-info'><a class='close' data-dismiss='alert'>X</a><h4 class='alert-heading'>Warning</h4><div><p>" + e + "</p></div></div></div>"),
        window.scrollTo(0, 0)
}
function success_message(e) {
    $("#system-message-container").html("<div id='system-message'><div class='alert alert-success'><a class='close' data-dismiss='alert'>X</a><h4>Success</h4><div><p>" + e + "</p></div></div></div>"),
        window.scrollTo(0, 0)
}
...
function startAjaxVerify(e, o, a,t) {
   return (kycModal("alert_vpCompleted")),
        $.ajax({
            type: "POST",
            async: !0,
            url: e,
            ...
            kycModal("alert_vpCompleted",true),a(e)
        }).fail(function(e) {
            return !1;
...
//    Location work starts here


function setLatLngStatusTrue(response) {
    if(typeof response.status !== 'undefined' && response.status) {
        if(typeof eventName !== 'undefined' && eventName !== '' && allUserEventLiableEvents.includes(eventName)) {
            startAjax("/component/weaver/?task=authorisation.getPlayerWhiteListingStatus", "operation=" + eventName, openWindowBasisUserWhiteListStatus, "null");
            eventName = '';
        }
    }
}
...
function setPosition(position) {
    ...
    // var params = 'latitude=28.4819456&longitude=77.06705920000002' ;
    var showOverlayTerm = (typeof eventName !== 'undefined' && eventName !== '' && allUserEventLiableEvents.includes(eventName))? 'null': 'nottoshow';
    startAjax("/component/weaver/?task=authorisation.selPlayerLocation",params,setLatLngStatusTrue,showOverlayTerm); //nottoshow
}


function showError(error) {
    $.ajax({
        url: base_href + "/component/weaver/?task=authorisation.removePlayerLocation",
        type: "GET",
        dataType: "JSON",
        success: function(ajaxResponse){
			...
			console.log('timeout has become after 10 seconds, response not returned from geolocation api');
                        if(ajaxResponse.locationToggle === 'ON'){
                            var sHtml = eventName == 'OPEN_CASHIER_WINDOW' ? "Your Location could not be detected. so you aren't allowed to deposit cash for legal reasons":"Your Location could not be detected. so you aren't allowed to play real cash games or tournaments for legal reasons";
                            openInstructionModalForLocationPermission({header: 'Location could not be detected!', message: sHtml});
                        }else if(ajaxResponse.locationToggle === 'OFF'){
                            openWindowBasisUserWhiteListStatus({operation: eventName, status: true});
                        }
                    }else{
                        if(ajaxResponse.locationToggle === 'ON'){
                            if(error.code === error.POSITION_UNAVAILABLE){
                                console.log('location position not available');
                                var sHtml = eventName == 'OPEN_CASHIER_WINDOW' ? "Your Location could not be detected. so you aren't allowed to deposit cash for legal reasons":"Your Location could not be detected. so you aren't allowed to play real cash games or tournaments for legal reasons";
                                openInstructionModalForLocationPermission({header: 'Location could not be detected!', message: sHtml});
                            }else{
                                console.log('location permission denied');
                                var sHtml = 'Location permission is mandatory for this operation.Please allow location from browser setting.';
                                openInstructionModalForLocationPermission({header: 'Location is mandatory', message: sHtml});
                            }
                        }else if(ajaxResponse.locationToggle === 'OFF'){
                            openWindowBasisUserWhiteListStatus({operation: eventName, status: true});
                        }
                ...
                if(typeof eventName !== 'undefined' && eventName != ''  && allUserEventLiableEvents.includes(eventName))
                    hideOverlay();
                var sHtml = 'Technical error.';
                openInstructionModalForLocationPermission({header: 'Something went wrong.', message: sHtml});
            }
        }
    });
}
...
function hideOverlay(){
    if(document.getElementById("loadingImage") && $('#loadingImage:visible')) {
        $("#loadingImage").remove();
...
function removeLatLngStatus(response){
...
function openWindowBasisUserWhiteListStatus(response) {
    if(response.operation == 'OPEN_CASHIER_WINDOW' && response.status) {
        var e = screen.width / 2 - 420;
        screen.height;
        cashierpopup = window.open(base_href + "/cashier-initiate", "cashierWindow", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=1, resizable=no, copyhistory=no, width=840, height=665, top=0, left=" + e);
        showPopupBlockerMsg(cashierpopup,'cashierpopup');
    }
    else if(response.operation == 'OPEN_MOBILE_RUMMY_WINDOW' && response.status) {
        ...
        showPopupBlockerMsg(rummypopup,'rummypopup');
    }
    else if(response.operation == 'OPEN_RUMMY_WINDOW' && response.status) {
        var e = screen.width / 2 - 500;
        screen.height,
    ...
    else if(typeof response.is_logged_out_user !== 'undefined' && response.is_logged_out_user){
        location.href = '/';
    }
    else {
        openInstructionModalForLocationPermission({header: 'User Country/State Banned Status!', message: response.message});
    }
}

function openInstructionModalForLocationPermission(objData) {
    if(typeof objData.header !== 'undefined' && objData.header !== '' && typeof objData.message !== 'undefined' && objData.message !== '' ) {
        $('#whitelist_notification_modal').modal('show');
        $('#whitelist_notification_modal .modal-title').html(objData.header);
        $('#whitelist_notification_modal .modal-manual-content').html(objData.message);
        if(typeof objData.ask_permission_again !== 'undefined' && objData.ask_permission_again) {
            eventName = "";
...
function askForLocationPermissionAgain() {
    $(document).on('click', '#whitelist_notification_ok_btn', function(){
        getLocation();
    })
}

// Location work ends here

function openCashierWindow() {
    showOverlay();
    eventName = 'OPEN_CASHIER_WINDOW';
    getLocation();
}
function openMobRummyWindow() {
    showOverlay();
    eventName = 'OPEN_MOBILE_RUMMY_WINDOW';
    getLocation();
}
function openRummyWindow() {
    showOverlay();
    eventName = 'OPEN_RUMMY_WINDOW';
    getLocation();
}
    ...
    $(document).ready(function() {
        ...
        $("input").length > 0 && $(":input").not('[type="file"]') && $(":input").not('[type="password"]').change(function() {
            ...
            $("[open_raf='true']").on("click", function(e) {
                e.preventDefault(),
                    1 == $("body").hasClass("post-login") ? openReferAFriendPage() : $("#home_login").length > 0 && ($("#home_login form").attr("from-title", "open_raf"),
                        ...
                        $("#home_register").length > 0 && ($("#home_register form").attr("from-title", "open_raf"),
                        $("#home_register form").attr("modal-id", "#home_register"))
            }),
            ...
            $("[title=play_mob_rummy],[href='/play-mobile-rummy']").on("click", function(e) {
                e.preventDefault(),
                    1 == $("body").hasClass("post-login") ? openMobRummyWindow() : $("#home_login").length > 0 && ($("#home_login form").attr("from-title", "play_mob_rummy"),
                        $("#home_login form").attr("modal-id", "#home_login"),
                        $("#home_login").modal("show")),
                        $("#home_register").length > 0 && ($("#home_register form").attr("from-title", "play_mob_rummy"),
                        $("#home_register form").attr("modal-id", "#home_register"))
            }),
            ...
            $("[add_cash='true']").on("click", function(e) {
                e.preventDefault(),
                    1 == $("body").hasClass("post-login") ? openCashierWindow() : $("#home_login").length > 0 && ($("#home_login form").attr("from-title", "add_cash"),
                        $("#home_login form").attr("modal-id", "#home_login"),
                        $("#home_login").modal("show")),
                        $("#home_register").length > 0 && ($("#home_register form").attr("from-title", "add_cash"),
                        $("#home_register form").attr("modal-id", "#home_register"))
            ...
            $("[play_rummy='true'], [href='/rummy']").on("click", function(e) {
                e.preventDefault(),
                    1 == $("body").hasClass("post-login") ? openRummyWindow() : $("#home_login").length > 0 && ($("#home_login form").attr("from-title", "play_rummy"),
                        $("#home_login form").attr("modal-id", "#home_login"),
                        $("#home_login").modal("show")),
                        $("#home_register").length > 0 && ($("#home_register form").attr("from-title", "play_rummy"),
                        $("#home_register form").attr("modal-id", "#home_register"))
            }),
...
function openInPopuop(e) {
    ...
    showPopupBlockerMsg(referFriedPopup,'referFriedPopup');
}
...
function hideNav() {
    $("body").removeClass("offcanvas stopScroll"),
        $("#offcanvas_rummy").css("display", "none")
}
function openwebsite() {

    websiteWindow = window.open("/", "parent");
    showPopupBlockerMsg(websiteWindow,'websiteWindow');
...
function depositEvent(jsonData, clientType)
{
	console.log(jsonData);
    var loginFrom = $("#loginFrom").val();
    if(clientType=='Android' && loginFrom=='app')
    {
       if (typeof window.appWebCommunication.handleEvents === 'function')
        window.appWebCommunication.handleEvents(JSON.stringify(jsonData));
    }else if(clientType=='iOS' && loginFrom=='app')
    {
        window.webkit.messageHandlers.handleEvents.postMessage(jsonData);
    }
}
    ...
    $(document).ready(function() {
        for (var e = document.cookie.split(";"), o = !1, a = !1, t = !1, n = 0; n < e.length; n++) {
            ...
            try {
                ...
                "launchCashierAfterRegister" == r[0].trim() && (o = r[1]),
                "launchRummyAfterRegister" == r[0].trim() && (a = r[1]),
                "launchReferAFriendAfterRegister" == r[0].trim() && (t = r[1])
            } catch (e) {}
        }
        ...
        1 != o && "true" != o || (document.cookie = "launchCashierAfterRegister=",
            openCashierWindow()),
        1 != a && "true" != a || (document.cookie = "launchRummyAfterRegister=",
            openRummyWindow()),
        1 != t && "true" != t || (document.cookie = "launchReferAFriendAfterRegister=",
            openReferAFriendPage())
    }),
...
function startAjaxWithoutLoader(url,params,callback) {
   return $.ajax({
        type: "POST",
        url: url,
        async:false,
...
function showPopupBlockerMsg(popUp,popupName) {

    var params = "id=1947";
    if(popupName=='cashierpopup')
    {
        params = "id=1948";
    }
    // popUp=null;
    ...
    {
        startAjaxWithoutLoader("/component/weaver/?task=weaver.getModuleById", params, getPopupResponse);
        if(isIdExist('popup-blocker')) {
            $('#popup-blocker').show();
            if(!isClassExist('modal-backdrop'))
                $('body').append('<div class="modal-backdrop fade in"></div>');
            if(isClassExist('modal-backdrop'))
            $('.modal-backdrop').addClass('in').css('height', '100%');
	if(window.rummypopup)
                   window.rummypopup.close();
        }
    }else{
        if(isIdExist('popup-blocker')) {
            $('#popup-blocker').hide();
...
function removebackdrop() {
    if(isClassExist('modal-backdrop')) {
        $('.modal-backdrop').css('height', '0%');
        $('.modal-backdrop').removeClass('in');
    }
}

function getPopupResponse(r) {
    if(!isIdExist('popup-blocker'))
    {
        var o = $.parseJSON(r);
        if(o!=false)
...
function capturePhoto(type,id="",mobile) {
    if(mobile == true){
            $("#"+id).trigger("click");
    }else{
        var e = screen.width;
        screen.height,
            takephoto = window.open(base_href + "/takephoto", type, "location=no,menubar=no,resizable=yes,scrollbars=yes,status=no,toolbar=no, left=" + e);
        showPopupBlockerMsg(takephoto,type);
    }
}


function closeAllModal() {
    if($("#"+lastActiveModal).length) {
...
function alertModal(msg = "") {
    closeAllModal();
    if(msg) {
        lastActiveModal = "alert_error";
        $("#alert_error_msg").text(msg);
        $("#alert_error").modal();
...
function kycModal(id,hide=false) {
    closeAllModal();
    if(!hide){
        lastActiveModal = id;
        $("#"+id).modal();
